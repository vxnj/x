...
tblBeg = '<table id="itemlist" class="tblX">' +'';

let datax = [];
function loadTbl(datax) {
    dataNice=''; rowsHtml='';
    datax.forEach(el => { 
        isFin = (el.fin<'2') ? ['itemopen', svgfin , svgdet] : ['itemdone', svgund , svgdel] ;
        owner = `img/usrPics/head-${el.usr}.png`;
        rowsHtml += 
            `<tr class="${isFin[0]}">
                <td>
                    <img class="ownerhead" src="${owner}"></img>
                    <input type="text" id="${el.id}" class="${isFin[0]}" value="${el.item}">
                    <span id="${el.id}">${isFin[1]}</span>
                    <span id="${el.id}">${isFin[2]}</span>
                </td>
