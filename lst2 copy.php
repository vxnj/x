...
<header>
    ...
    <button onClick="refr()" >  refr  </button>
</header>
...
while($row = $result->fetch_assoc()) {
    $done=  ($row['fin']==0) ? 'class="itemopen" id="' .$row['id']. '"' : 'class="itemdone"';
    $btns = ($row['fin']==0) ? [$svgfin , $svgdet] : [$svgund , $svgdel] ;

    echo '<tr>
            <td ' . $done .'><input type="text" name="item" value ="' . $row['item'] . '"></td>
            <td>
                <span id='.$row["id"].'>'.$btns[0].'</span>
                <span id='.$row["id"].'>'.$btns[1].'</span>
            </td>
        </tr>';
}
?>  //php -->
...
<script>
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        items[i].addEventListener('touchend', doEditK );
    ...
    function refr() {
        console.log('refr'); 
        //document.getElementById("xyzz").outerHtml = "xyzz";
        $.ajax({ 
            type: "GET",
            url: "db/lst2f2.php",
            // data: "actn=qry&qry=SELECT * FROM listoSort limit 0,30",
            success: (function(output) {
                    console.log(output);
                    //inner = output;
                    document.getElementById("xyzz").innerHtml = output;
                    $("span").append(" <b>Appended text</b>.");

            }) // success
        }) //ajax
    } //refr

    //tbl = document.getElementById("itemlist");
